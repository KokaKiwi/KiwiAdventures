...
public class World
{
    ...
    private final org.jbox2d.dynamics.World                       jboxWorld;
     ...
     * own them
     */
    private final Map<org.jbox2d.collision.shapes.Shape, Body<?>> shapeMap            = new HashMap<org.jbox2d.collision.shapes.Shape, Body<?>>();
    ...
    private final Map<Body<?>, Set<WorldListener>>                bodyListeners       = new HashMap<Body<?>, Set<WorldListener>>();
    ...
    public void add(Body<?> body)
    {
        ...
        final List<org.jbox2d.collision.shapes.Shape> shapes = body.getShape()
                .getJBoxShapes();
    ...
    private class BodyQueryCallbackHelper implements QueryCallback
    {
        ...
        {
            final Body<?> body = shapeMap.get(fixture.getShape());
            if (body != null)
            {
                bodies.add(body);
            }
    ...
    private void resolveOutOfBounds()
    {
        ...
        {
            final Collection<Body<?>> bodies = new LinkedList<Body<?>>();
            final QueryCallback queryCallback = new BodyQueryCallbackHelper(
                    bodies);
            for (final AABB region : outOfBoundsRegions)
            {
            ...
            {
                final boolean continueProcessing = outOfBoundsCallback
                        .reportBodies(bodies);
    ...
    {
        final Set<WorldListener> listeners = bodyListeners.get(body);
        if (listeners != null)
        {
            listeners.remove(listener);
    ...
    public List<Body<?>> bodiesAt(float x1, float y1, float x2, float y2)
    {
        ...
        final AABB aabb = new AABB(new Vec2(lowerX, lowerY), new Vec2(upperX,
                upperY));
    ...
    {
        final CollisionEvent event = new CollisionEvent(bodyA, bodyB, contact);
        ...
        {
            for (final WorldListener listener : moreListeners)
            {
                listener.collided(event);
            }
        }
    ...
    private class ProxyContactListener implements ContactListener
    {
        ...
        {
            final Body<?> bodyA = shapeMap
                    .get(contact.getFixtureA().getShape());
            final Body<?> bodyB = shapeMap
                    .get(contact.getFixtureB().getShape());
