...
public class Rectangle extends PolygonBasedShape
{
            ...
            float restitution, float friction)
    {
        ...
        shape.setAsBox(width * World.METERS_PER_PIXEL / 2, height
                * World.METERS_PER_PIXEL / 2,
        // then we set it so that the origin is at the upper left corner so
        // people like us
                new Vec2(+width * World.METERS_PER_PIXEL / 2, +height
                        * World.METERS_PER_PIXEL / 2), 0);
        def.density = density;
    ...
    protected void applyOffset(float x, float y, float angle)
    {
        ...
        // then we set it so that the origin is at the upper left corner so
        // people like us
                new Vec2(x * World.METERS_PER_PIXEL + width
                        * World.METERS_PER_PIXEL / 2, y
                        * World.METERS_PER_PIXEL + height
                        * World.METERS_PER_PIXEL / 2), 0);
    }
}
