...
/**
 * A single body in the world. A body holds a shape which collide with the rest
 * of the world. It also holds properties about the shapes once they have been
 * created.
 * 
...
abstract public class Body<T>
{
    ...
    private final List<Body<?>>        touching = new ArrayList<Body<?>>();
     ...
     * 
     * @param shape
     *            The shape the body should have
     * @param x
     *            The x axis location of the body
     * @param y
     *            The y axis location of the body
     */
    public Body(Shape shape, float x, float y)
    {
        jboxBodyDef = new BodyDef();
        jboxBodyDef.position = new Vec2(x * METERS_PER_PIXEL, y
                * METERS_PER_PIXEL);
        this.shape = shape;
    }
     ...
     * 
     * @return Get the user data assigned to this body (or null if none is
     *         defined);
     */
     ...
     * 
     * @param object
     *            The user data to be assigned to this body
     */
     ...
     * 
     * @param other
     *            The other body to check against
    ...
    public int touchCount(Body<?> other)
    {
        ...
        for (final Body<?> touched : touching)
        {
            if (touched == other)
            {
                count++;
    ...
    /**
     * Indicate that this body touches another at a *new* contact point.
     * 
     * @param other
     *            The other body that is touched
     */
     ...
     * 
     * @param other
     *            The other body that is no longer touched by a particular
     *            contact point.
     */
    ...
    /**
     * Reset all touch-related data -- disappear from other objects' touch
     * information.
     */
    void resetTouching()
    {
     ...
     * 
     * @param x
     *            The amount of force on the X axis
     * @param y
     *            The amount of force on the Y axis
     */
     ...
     *            The amount of force on the Y axis
     * @param xAt
     *            The x-coordinate at which to apply the force
     * @param yAt
     *            The y-coordinate at which to apply the force
     * @param isLocalPoint
     *            if true, assume the *At params are local to the body.
     *            otherwise, they're world coordinates.
     */
    public void applyForce(float xMagnitude, float yMagnitude, float xAt,
            float yAt, boolean isLocalPoint)
    {
        assertBodyAttached();
     ...
     * 
     * @param torque
     *            about the z-axis (out of the screen), usually in N.
     */
    ...
    {
        assertBodyAttached();
        jboxBody.applyLinearImpulse(new Vec2(xMagnitude, yMagnitude),
                jboxBody.getWorldCenter());
    }
     ...
     * 
     * @param xMagnitude
     *            impulse strength on the X axis
     * @param yMagnitude
     *            impulse strength on the Y axis
     * @param xAt
     *            The x-coordinate at which to apply the impulse
     * @param yAt
     *            The y-coordinate at which to apply the impulse
     * @param isLocalPoint
     *            if true, assume the *At params are local to the body.
     *            otherwise, they're world coordinates.
     */
    public void applyImpulse(float xMagnitude, float yMagnitude, float xAt,
            float yAt, boolean isLocalPoint)
    {
        assertBodyAttached();
        Vec2 position = new Vec2(xAt * METERS_PER_PIXEL, yAt * METERS_PER_PIXEL);
    ...
    /**
     * (from JBox2d): Apply an impulse. This affects the angular velocity
     * without affecting the linear velocity of the center of mass. This wakes
     * up the body.
     * 
     * @param impulse
     *            about the z-axis (out of the screen), in newton*seconds.
     */
    ...
    public float getX()
    {
        return attached ? jboxBody.getPosition().x * PIXELS_PER_METER
                : jboxBodyDef.position.x * PIXELS_PER_METER;
    }
    ...
    public float getY()
    {
        return attached ? jboxBody.getPosition().y * PIXELS_PER_METER
                : jboxBodyDef.position.y * PIXELS_PER_METER;
    }
     ...
     * 
     * @param rest
     *            The restitution applied when this body collides
     */
     ...
     * 
     * @param f
     *            The friction applied when this body collides
     */
    ...
    /**
     * Tests whether this body is out of bounds. Since the world no longer
     * tracks which objects is out of bounds, this just checks whether the body
     * is still active...which will actually work as expected if the world's
     * {@link World.OutOfBoundsBehavior} is set to DESTROY or DEACTIVATE.
     * 
     * @deprecated use {@link #isActive()}, {@link #isAttached()}, and
     *             {@link World#setOutOfBoundsBehavior(org.newdawn.fizzy.World.OutOfBoundsBehavior)}
     *             instead.
     ...
     * 
     * @param world
     *            The world this body is being added to
     */
    void addToWorld(World world)
    {
        final org.jbox2d.dynamics.World jboxWorld = world.getJBoxWorld();
     ...
     * 
     * @param world
     *            The world this body is being removed from
     */
    ...
    /**
     * Set the position of the body. This can only be called after the body has
     * been added to the world.
     * 
     * @param x
     *            The new x coordinate of the body
     * @param y
     *            The new y coordinate of the body
     */
    public void setPosition(float x, float y)
    {
        assertBodyAttached();
        jboxBody.setTransform(new Vec2(x * METERS_PER_PIXEL, y
                * METERS_PER_PIXEL), jboxBody.getAngle());
    }
    
    /**
     * Set the rotation of the body. This can only be called after the body has
     * been added to the world.
     * 
     * @param rotation
     ...
     * 
     * @param x
     *            The amount to move the body on the x axis
     * @param y
     *            The amount to move the body on the y axis
     */
    ...
    /**
     * Set the linear damping to apply to this body. Higher value slows the body
     * acceleration. Maximum is 1.0.
     * 
     * @param damping
     *            The amount to dampen the movement by
    ...
    public void setLinearDamping(float damping)
    {
        if (jboxBody != null)
        {
            throw new AlreadyAddedToWorldException();
        }
        jboxBodyDef.linearDamping = damping;
    }
    
    /**
     * Set the angular damping to apply to this body. Higher value slows the
     * angular velocity faster. Maximum is 1.0.
     * 
     * @param damping
     *            The amount to dampen the rotation by
     */
    ...
    {
        assertBodyAttached();
        final Vec2 vel = jboxBody.getLinearVelocity();
        vel.x = xVelocity;
        vel.y = yVelocity;
    ...
    /**
     * Sets whether this body has fixed rotation enabled. Bodies with Fixed
     * Rotation can't rotate...in case that wasn't obvious.
     * 
     * @param fixedRotation
     *            true if fixed rotation mode should be enabled.
     */
    ...
    /**
     * Get the bounding box that encloses this body and all of its constituent
     * shapes
     * 
    ...
    {
        final AABB bodyAABB = new AABB(new AABB(new Vec2(Float.MAX_VALUE,
                Float.MAX_VALUE), new Vec2(Float.MIN_VALUE, Float.MIN_VALUE)));
        final AABB shapeAABB = new AABB();
        for (final org.jbox2d.collision.shapes.Shape jshape : shape
                .getJBoxShapes())
        {
    ...
    /**
     * Is the current body considered a "bullet" (at high risk of passing
     * through other objects due to velocity). This is on by default for
     * Dynamic<->Static interactions but not Dynamic<->Dynamic interactions.
     * 
     ...
     * @see http://www.box2d.org/manual.html
     * @param flag
     *            true if body is to be active, false otherwise.
     */
    ...
    /**
     * sets the world this object is in. should really only be done when
     * attached to world
     */
